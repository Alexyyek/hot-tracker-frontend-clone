# AI Hot Tracker 当前信息源清单

生成时间：2026-07-03

说明：
- 这里按当前 `src/aiTopics.ts` 的主题目录整理。
- `配置数` 为该主题下去重后的候选信息源数。
- `当前有内容` 为最近一次静态快照中该主题下有内容的源数量。
- 最近一次快照由 `local-source-collector` 独立生成，共 243 条内容；生产站点不再依赖原 Hot Tracker API。
- GitHub 热榜已按用户要求禁用。
- 本次新增/确认的 9 个公众号已进入候选目录；其中 7 个已通过独立微信搜索通道抓到文章。`公众号：阿里技术`、`公众号：阿里云开发者` 当前仍未命中精确账号文章。
- X 源已保留在目录中，但默认需要 `X_BEARER_TOKEN` 后才会采集；无 token 时显示为 0 条。
- 当前目录内仍有少量重复配置：`公众号：生数科技（Vidu·视频）`、`公众号：可灵AI（快手·视频）`、`公众号：蚂蚁百灵（Ling）`、`公众号：面壁智能（MiniCPM）`、`公众号：阶跃星辰（Step）`。下方清单已按主题内去重展示。

## 总览

| 主题 | 配置数 | 当前有内容 |
| --- | ---: | ---: |
| AI 行业进展 | 51 | 31 |
| AI 论文前沿 | 26 | 16 |
| AI 应用案例 | 55 | 40 |
| 大厂 AI 日报 | 93 | 63 |
| GitHub AI 热榜（已禁用） | 0 | 0 |

## 新增微信源验证结果

| 信息源 | 当前条数 | 状态 |
| --- | ---: | --- |
| 公众号：PaperWeekly | 3 | 已通过微信搜索抓取 |
| 公众号：机器学习实验室 | 2 | 已通过微信搜索抓取 |
| 公众号：智能涌现 | 3 | 已通过微信搜索抓取 |
| 公众号：高德技术 | 2 | 已通过微信搜索抓取 |
| 公众号：阿里技术 | 0 | 搜狗精确账号当前未命中 |
| 公众号：阿里云开发者 | 0 | 搜狗精确账号当前未命中 |
| 公众号：DataFunTalk | 3 | 已通过微信搜索抓取 |
| 公众号：AI科技大本营 | 3 | 已通过微信搜索抓取 |
| 公众号：微软亚洲研究院 | 2 | 已通过微信搜索抓取 |

## AI 行业进展

配置数：51；当前有内容：31

### 网站 / 博客 / API

- Hugging Face
- OpenAI
- Anthropic
- Google DeepMind
- xAI
- Qwen：Blog Retrieval（API）
- Qwen：Research（API）
- 字节 Seed：Research Feed（网页内嵌数据）
- NVIDIA AI Blog
- NVIDIA Technical Blog（开发者技术博客 · RSS）
- Claude
- The Decoder：AI News（RSS）
- Nathan Lambert：Interconnects（RSS）
- Google Research：Blog（网页）
- Artificial Intelligence News（RSS）
- Thinking Machines Lab：官方博客（RSS）

### X

- X：Rohan Paul (@rohanpaul_ai)
- X：Nathan Lambert (@natolambert)
- X：Noam Brown (@polynoamial)
- X：Ilya Sutskever (@ilyasut)
- X：Demis Hassabis (@demishassabis)
- X：Dario Amodei (@DarioAmodei)
- X：Artificial Analysis (@ArtificialAnlys)
- X：阿易 AI Notes (@AYi_AInotes)
- X：Claude (@claudeai)
- X：Perplexity (@perplexity_ai)
- X：Microsoft Research (@MSFTResearch)
- X：硅基流动 SiliconFlow (@SiliconFlowAI)
- X：Sundar Pichai (@sundarpichai)
- X：谢赛宁 (@sainingxie)
- X：Elvis Saravia (@omarsar0, DAIR.AI)
- X：SemiAnalysis (@SemiAnalysis_)
- X：swyx (@swyx)
- X：Deedy Das (@deedydas)
- X：Eric Mitchell (@ericmitchellai)

### 微信公众号

- 公众号：卡尔的AI沃茨
- 公众号：数字生命卡兹克
- 公众号：机器之心
- 公众号：量子位
- 公众号：新智元
- 公众号：AI科技评论
- 公众号：PaperWeekly
- 公众号：机器学习实验室
- 公众号：智能涌现
- 公众号：夕小瑶科技说
- 公众号：AINLP
- 公众号：DataFunTalk
- 公众号：AI科技大本营
- 公众号：微软亚洲研究院
- 公众号：阿里云开发者
- 公众号：小红书技术（dots.llm）

## AI 论文前沿

配置数：26；当前有内容：16

### 网站 / 博客 / API

- HuggingFace Daily Papers（社区热门论文）
- arXiv：Agent Harness / Auto-Research
- Anthropic：Transformer Circuits（可解释性研究）
- 智谱：研究（网页内嵌数据）
- Qwen：Research（API）
- 字节 Seed：Research Feed（网页内嵌数据）
- Berkeley RDI：Blog（AI 安全与评测）
- Lilian Weng：Lil'Log（RSS）
- Google Research：Blog（网页）
- Andrej Karpathy：Blog（网页）
- CMU：Machine Learning Blog
- BAIR：Berkeley AI Research Blog

### X

- X：Andrej Karpathy (@karpathy)
- X：Sky Computing Lab (@haoailab)
- X：Epoch AI (@EpochAIResearch)
- X：Microsoft Research (@MSFTResearch)
- X：Nathan Lambert (@natolambert)
- X：Noam Brown (@polynoamial)
- X：Jeff Dean (@JeffDean)
- X：Yann LeCun (@ylecun)
- X：Jason Liu (@jxnlco)
- X：Lilian Weng (@lilianweng)

### 微信公众号

- 公众号：PaperWeekly
- 公众号：机器学习实验室
- 公众号：智能涌现
- 公众号：微软亚洲研究院

## AI 应用案例

配置数：55；当前有内容：40

### 网站 / 博客 / API

- OpenRouter
- arXiv：Agent Harness / Auto-Research
- Cursor
- Simon Willison 博客
- Steve Yegge：Medium（RSS）
- Google Developers
- 蚂蚁百灵：Developer Blog（网页）
- NVIDIA Technical Blog（开发者技术博客 · RSS）
- MiniMax：Blog（网页）
- NVIDIA Blog：Agentic AI（网页）
- MiniMax：News（网页）
- Moonshot AI：Kimi Blog（VitePress）

### X

- X：OpenRouter (@OpenRouter)
- X：OpenAI Developers (@OpenAIDevs)
- X：Claude Devs (@ClaudeDevs)
- X：Replit (@Replit)
- X：Boris Cherny (@bcherny)
- X：opencode (@opencode)
- X：Jason Liu (@jxnlco)
- X：邵猛 (@shao__meng)
- X：小互 (@xiaohu)
- X：Google AI for Developers (@googleaidevs)
- X：阶跃星辰 StepFun (@StepFun_ai)
- X：DeepSeek (@deepseek_ai)
- X：Kimi.ai (@Kimi_Moonshot)

### 微信公众号

- 公众号：月之暗面（Kimi）
- 公众号：DeepSeek（深度求索）
- 公众号：千问APP（阿里）
- 公众号：智谱（GLM）
- 公众号：面壁智能（MiniCPM）
- 公众号：蚂蚁百灵（Ling）
- 公众号：阶跃星辰（Step）
- 公众号：昆仑万维（天工）
- 公众号：生数科技（Vidu·视频）
- 公众号：可灵AI（快手·视频）
- 公众号：数字生命卡兹克
- 公众号：机器之心
- 公众号：量子位
- 公众号：新智元
- 公众号：AI科技评论
- 公众号：AI科技大本营
- 公众号：InfoQ
- 公众号：PaperWeekly
- 公众号：DataFunTalk
- 公众号：智能涌现
- 公众号：高德技术
- 公众号：阿里技术
- 公众号：阿里云开发者
- 公众号：机器学习实验室
- 公众号：微软亚洲研究院
- 公众号：京东JoyAI
- 公众号：MiniMax（稀宇科技）
- 公众号：豆包（字节）
- 公众号：腾讯元宝
- 公众号：小红书技术（dots.llm）

## 大厂 AI 日报

配置数：93；当前有内容：63

### 网站 / 博客 / API

- OpenAI
- Anthropic
- Google DeepMind
- xAI
- 大厂日爆
- Anthropic：Transformer Circuits（可解释性研究）
- NVIDIA Technical Blog（开发者技术博客 · RSS）
- Google Blog：AI（RSS）
- NVIDIA AI Blog
- 智谱：研究（网页内嵌数据）
- Qwen：Blog Retrieval（API）
- Qwen：Research（API）
- MiniMax：Blog（网页）
- NVIDIA Blog：Agentic AI（网页）
- NVIDIA Blog：Generative AI（网页）
- 字节 Seed：Research Feed（网页内嵌数据）
- MiniMax：News（网页）
- Moonshot AI：Kimi Blog（VitePress）
- Google Developers
- Meta
- Meta Engineering Blog（RSS）

### X

- X：阿里云 / Alibaba Cloud (@alibaba_cloud)
- X：OpenAI Developers (@OpenAIDevs)
- X：OpenAI (@OpenAI)
- X：Anthropic (@AnthropicAI)
- X：Google AI for Developers (@googleaidevs)
- X：xAI (@xai)
- X：Google DeepMind (@GoogleDeepMind)
- X：腾讯混元 (@TencentHunyuan)
- X：阶跃星辰 StepFun (@StepFun_ai)
- X：百度 Baidu (@Baidu_Inc)
- X：Google AI (@GoogleAI)
- X：NVIDIA AI (@NVIDIAAI)
- X：DeepSeek (@deepseek_ai)
- X：NVIDIA (@nvidia)
- X：智谱 Z.ai (@Zai_org)
- X：Kimi.ai (@Kimi_Moonshot)
- X：AI at Meta (@AIatMeta)
- X：华为云 (@HuaweiCloud1)
- X：通义千问 / Qwen (@Alibaba_Qwen)
- X：蚂蚁百灵 (@AntLingAGI)
- X：小米 MiMo (@XiaomiMiMo)

### 微信公众号

- 公众号：月之暗面（Kimi）
- 公众号：DeepSeek（深度求索）
- 公众号：千问APP（阿里）
- 公众号：智谱（GLM）
- 公众号：面壁智能（MiniCPM）
- 公众号：蚂蚁百灵（Ling）
- 公众号：阶跃星辰（Step）
- 公众号：昆仑万维（天工）
- 公众号：数字生命卡兹克
- 公众号：机器之心
- 公众号：量子位
- 公众号：新智元
- 公众号：AI科技评论
- 公众号：AI科技大本营
- 公众号：InfoQ
- 公众号：PaperWeekly
- 公众号：AINLP
- 公众号：DataFunTalk
- 公众号：机器学习实验室
- 公众号：智能涌现
- 公众号：微软亚洲研究院
- 公众号：百度智能云（文心）
- 公众号：腾讯混元
- 公众号：京东JoyAI
- 公众号：MiniMax（稀宇科技）
- 公众号：豆包（字节）
- 公众号：腾讯元宝
- 公众号：龙猫LongCat（美团）
- 公众号：小米 MiMo
- 公众号：字节跳动技术团队
- 公众号：阿里技术
- 公众号：阿里云开发者
- 公众号：高德技术
- 公众号：腾讯技术工程
- 公众号：腾讯云开发者
- 公众号：美团技术团队
- 公众号：百度Geek说
- 公众号：百度智能云技术
- 公众号：快手技术
- 公众号：京东云开发者
- 公众号：华为云开发者联盟
- 公众号：蚂蚁技术AntTech
- 公众号：小红书技术
- 公众号：小红书技术（dots.llm）
- 公众号：携程技术
- 公众号：网易云音乐技术团队
- 公众号：网易数帆
- 公众号：得物技术
- 公众号：vivo互联网技术
- 公众号：OPPO数智技术
- 公众号：哔哩哔哩技术
