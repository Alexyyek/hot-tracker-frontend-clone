...
export function getAiTopicCounts(items: FeedItem[], sourceFacets: SourceFacet[], backendCounts: TopicCount[]) {
  const counts = aiTopics.map((topic) => ({
    topicId: topic.id,
    count: topic.id === "ai-big-tech"
      ? getBackendTopicCount(backendCounts, topic.sourceTopicId) || countSourcesForTopic(sourceFacets, topic.id)
      : countSourcesForTopic(sourceFacets, topic.id) || filterAiFeedItems(items, topic.id).length
  }));

  return [{ topicId: AI_ALL_TOPIC_ID, count: countSourcesForTopic(sourceFacets) || filterAiFeedItems(items).length }, ...counts];
}
